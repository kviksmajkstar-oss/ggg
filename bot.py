...
async def analyze_user_message(chat_id: int, user_text: str) -> str:
    ...
    try:
        ...
        print(f"[ERROR] OpenAI text failed: {e}", flush=True)
        text = "Я с тобой 💛 Сейчас у меня проблемы с ИИ-сервисом, но я всё равно рядом. Расскажи ещё чуть-чуть, я поддержу. "
