...
async def analyze_user_message(chat_id: int, user_text: str) -> str:
    ...
    for model_name in [MODEL, FALLBACK_MODEL]:
        try:
            ...
            last_error = e
            print(f"[ERROR] OpenAI text failed ({model_name}): {e}", flush=True)
    ...
    if text is None:
        ...
        if e and is_invalid_api_key_error(e):
            ...
            text = f"Я тебя услышала 💛 Пока ИИ недоступен, но я рядом. Главное из твоего сообщения: «{user_short}». Хочешь, поддержу тебя прямо сейчас?"
